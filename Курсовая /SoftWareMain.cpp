...
void SearchSong(HWND hWnd) {
	...
	// ищем слово в тексте
	if (strstr(Buffer, query.c_str())) {  // слово найдено
...
void SearchSong(HWND hWnd) {
	...
	FILE* file = fopen("G:\\Данила\\С++\\6 practic\\Ex 2\\Курсач\\base.txt", "r"); // открываем файл для чтения
	...
	while (fgets(Buffer, TextBufferSize, file)) { // читаем строки из файла
		if (strstr(Buffer, query.c_str())) { // ищем слово в строкеs
			found = true;
			break;
		}
	}
